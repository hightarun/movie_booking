...
const Login = () => {
  ...
  const submitForgot = async (e) => {
    ...
    setForgotModal(true);
    setResetFormUser(
      e.target.elements.emailOrUname.name,
      e.target.elements.emailOrUname.value
    );
    try {
      let res = await axios({
        method: "get",
        url: `${process.env.REACT_APP_BACKEND}/${forgotUser}/forgot`,
      });
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    dispatch(loginUser(formdata.emailOrUname, formdata.password));
  };
